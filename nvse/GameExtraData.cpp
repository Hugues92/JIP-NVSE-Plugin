...
ExtraScript* ExtraScript::Create(TESForm* baseForm, bool create, TESObjectREFR* container) {
	...
	if (xScript && baseForm) {
		...
		if (pScript && pScript->script) {
			...
			if (create) {
				// create Event struct
				ScriptEventList::Event * pEvent = (ScriptEventList::Event*)FormHeap_Allocate(sizeof(ScriptEventList));
				pEvent->eventMask = ScriptEventList::kEvent_OnAdd;
				pEvent->object = container;

				// create ScriptEventList
				xScript->eventList = xScript->script->CreateEventList();
				if (xScript->eventList) {
					if (!xScript->eventList->m_eventList) {
						xScript->eventList->m_eventList = (ScriptEventList::EventList*)FormHeap_Allocate(sizeof(ScriptEventList::EventList));
						xScript->eventList->m_eventList->Init();
						xScript->eventList->m_eventList->AddAt(pEvent, 0);
					}
				}
			}
		}
